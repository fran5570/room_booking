...
class ReservationController:
    ...
    def create_reservation(
        self, user_id: int, room_id: int, date: str, start_time: str, end_time: str
    ):
    ...
    def list_reservations(self):
        ...
        if results:
            ...
            for row in results:
                print(
                    f"[{row[0]}] {row[1]} booked '{row[2]}' on {row[3]} from {row[4]} to {row[5]}"
                )
        else:
    ...
    def list_reservations_by_user(self, user_id: int):
        ...
        if results:
            ...
            for row in results:
                print(
                    f"[{row[0]}] Room: {row[1]}, Date: {row[2]}, {row[3]} to {row[4]}"
                )
        else:
    ...
    def list_reservations_by_room(self, room_id: int):
        ...
        if results:
            ...
            for row in results:
                print(
                    f"[{row[0]}] User: {row[1]}, Date: {row[2]}, {row[3]} to {row[4]}"
                )
        else:
